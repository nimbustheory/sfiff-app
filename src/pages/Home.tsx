...
// Venue Card Component with hover effect
function VenueCard({ venue, index, onClick }: { venue: { name: string; location: string; image: string }; index: number; onClick: () => void }) {
  const gradients = [
  ...
  return (
    <div 
      onClick={onClick}
      className="relative aspect-[3/4] rounded-xl overflow-hidden cursor-pointer group card-3d"
    >
      <div className="card-3d-inner w-full h-full">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-2 text-white z-10">
          <p className="font-semibold text-xs line-clamp-1">{venue.name}</p>
          <p className="text-white/70 text-[10px]">{venue.location}</p>
        </div>
      </div>
    </div>
  );
